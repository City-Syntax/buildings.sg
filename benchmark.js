document.addEventListener('DOMContentLoaded', () => {
    ...
    // 点击其他 nav 时自动关闭 benchmark 面板
    ['type-link', 'carbon-link', 'energy-link', 'bca-link', 'ura-link', 'data-link'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.addEventListener('click', () => bmPanel.classList.remove('show'));
    });
    ...
    function bmCalculateCore(subtype, archetype, userData) {
        ...
        chartConfigs.forEach(cfg => {
            ...
            const rawData = rawBuildingData
                .filter(item => normalizeBuildingType(item['Building Type']) === normalizeBuildingType(archetype))
    ...
    function bmRenderCharts(selectedArchetype) {
        ...
        const filteredData = rawBuildingData.filter(
            item => normalizeBuildingType(item['Building Type']) === normalizeBuildingType(selectedArchetype)
        );
    ...
    const BM_REPORT_LABELS = {
        "bm-archetype-select": "Archetype",
        "bm-eui-input": "Input EUI",
        "bm-gui-input": "Input GUI",
        "bm-gfa-input": "Input GFA",
        "bm-chiller-input": "Input Chilled Water Efficiency",
        "bm-airside-input": "Input Air-side Efficiency",
        "bm-rank-eui": "EUI Percentile Rank",
        "bm-rank-gui": "GUI Percentile Rank",
        "bm-dev-bench-eui": "EUI Deviation from Benchmark",
        "bm-dev-bench-gui": "GUI Deviation from Benchmark",
        "bm-dev-gm-eui": "EUI Deviation from Green Mark",
        "bm-dev-gm-gui": "GUI Deviation from Green Mark",
        "bm-notes-eui": "EUI Notes",
        "bm-notes-gui": "GUI Notes",
        "bm-rank-chiller": "Chiller Percentile Rank",
        "bm-rank-airside": "Air-side Percentile Rank",
        "bm-dev-bench-chiller": "Chiller Deviation from Benchmark",
        "bm-dev-bench-airside": "Air-side Deviation from Benchmark",
        "bm-dev-gm-chiller": "Chiller Deviation from Green Mark",
        "bm-dev-gm-airside": "Air-side Deviation from Green Mark",
        "bm-notes-chiller": "Chiller Notes",
        "bm-notes-airside": "Air-side Notes"
    };
    ...
    function addPdfFooter(doc, pageWidth, pageHeight) {
        ...
        doc.text('Generated by Buildings.sg Benchmark Module', 40, pageHeight - 18);
    }

    function exportReport() {
        console.log("Starting frontend PDF report export process...");

        const finalData = buildDataToSend();
        if (Object.keys(finalData).length === 0) {
            alert("Could not extract report data. Please ensure the page is fully loaded.");
            return;
        }
        ...
        try {
            ...
            const drawKVLine = (label, value) => {
                ...
                const wrapped = doc.splitTextToSize(safeValue, pageWidth - margin * 2 - 130);
            ...
            doc.roundedRect(margin, y - 24, pageWidth - margin * 2, 78, 8, 8, 'F');
            doc.setTextColor(255, 255, 255);
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(18);
            doc.text('Buildings.sg Benchmark Report', margin + 16, y + 2);
            doc.setFont('helvetica', 'normal');
            doc.setFontSize(10);
            doc.text('Generated: ' + new Date().toLocaleString(), margin + 16, y + 22);
            doc.text('Archetype: ' + (finalData['bm-archetype-select'] || 'N/A'), margin + 16, y + 40);
            y += 72;
            ...
            [
                ...
                'bm-chiller-input',
                'bm-airside-input'
            ].forEach((id) => drawKVLine(BM_REPORT_LABELS[id], finalData[id]));
            ...
            const metricBlocks = [
                {
                    ...
                    rank: finalData['bm-rank-eui'],
                    benchmark: finalData['bm-dev-bench-eui'],
                    greenmark: finalData['bm-dev-gm-eui'],
                    notes: finalData['bm-notes-eui']
                },
                {
                    title: 'Gas Use Intensity (GUI)',
                    rank: finalData['bm-rank-gui'],
                    benchmark: finalData['bm-dev-bench-gui'],
                    greenmark: finalData['bm-dev-gm-gui'],
                    notes: finalData['bm-notes-gui']
                },
                {
                    title: 'Chilled Water Efficiency',
                    rank: finalData['bm-rank-chiller'],
                    benchmark: finalData['bm-dev-bench-chiller'],
                    greenmark: finalData['bm-dev-gm-chiller'],
                    notes: finalData['bm-notes-chiller']
                },
                {
                    title: 'Air-side Efficiency',
                    rank: finalData['bm-rank-airside'],
                    benchmark: finalData['bm-dev-bench-airside'],
                    greenmark: finalData['bm-dev-gm-airside'],
                    notes: finalData['bm-notes-airside']
                }
            ];

            metricBlocks.forEach((block) => {
                const noteText = block.notes || 'N/A';
                const noteLines = doc.splitTextToSize('Notes: ' + noteText, pageWidth - margin * 2 - 20);
                const cardHeight = Math.max(92, 66 + noteLines.length * 11);

                ensureSpace(cardHeight + 12);

                doc.setFillColor(247, 247, 247);
                doc.setDrawColor(228, 228, 228);
                doc.roundedRect(margin, y, pageWidth - margin * 2, cardHeight, 7, 7, 'FD');

                doc.setFont('helvetica', 'bold');
                doc.setTextColor(40, 40, 40);
                doc.setFontSize(11);
                doc.text(block.title, margin + 10, y + 17);

                doc.setFont('helvetica', 'normal');
                doc.setFontSize(10);
                doc.text('Percentile Rank: ' + (block.rank || 'N/A'), margin + 10, y + 35);
                doc.text('Deviation from Benchmark: ' + (block.benchmark || 'N/A'), margin + 10, y + 50);
                doc.text('Deviation from Green Mark: ' + (block.greenmark || 'N/A'), margin + 10, y + 65);
                doc.text(noteLines, margin + 10, y + 80);

                y += cardHeight + 10;
            });

            // Keep original field read order in the appendix for traceability.
            ensureSpace(40);
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(12);
            doc.text('Appendix: Raw Fields (Original Read Order)', margin, y);
            y += 8;
            doc.setDrawColor(225, 225, 225);
            doc.line(margin, y, pageWidth - margin, y);
            y += 14;

            BM_REPORT_FIELD_IDS.forEach((id) => {
                drawKVLine(BM_REPORT_LABELS[id] || id, finalData[id]);
            });

            addPdfFooter(doc, pageWidth, pageHeight);

            const archetypeName = (finalData['bm-archetype-select'] || 'Benchmark').replace(/[^a-zA-Z0-9_-]/g, '_');
            const dateStamp = new Date().toISOString().slice(0, 10);
            doc.save(`Benchmark_Report_${archetypeName}_${dateStamp}.pdf`);
            console.log("PDF report successfully generated and download started.");

        } catch (error) {
            console.error("An error occurred during PDF report export:", error);
        ...
        BM_REPORT_FIELD_IDS.forEach(id => {
            ...
            if (element) {
                if (element.tagName === 'SELECT') {
                    ...
                    dataToSend[id] = selectedOption ? selectedOption.text.trim() : 'N/A';
                } else if (element.value !== undefined) {
                    dataToSend[id] = String(element.value).trim() || 'N/A';
                } else {
                    dataToSend[id] = element.innerText.trim() || 'N/A';
                }
            } else {
                // 统一的英文警告日志
