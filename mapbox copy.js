...
map.on('load', () => {
    ...
    // 添加数据源（更新）
    map.addSource('sg_buildings_v2-7mhw61', {
        type: 'vector',
        url: 'mapbox://citysyntaxlab.3up4onl2'
    });
    ...
    map.addLayer([
        {
            ...
            "type": "fill-extrusion",
            "source": "mapbox://citysyntaxlab.3up4onl2",
            "id": "sg-buildings-3d",
            "paint": {
            ...
            "slot": "",
            "source-layer": "sg_buildings_v2-7mhw61"
        }
    ]);

    // 全局建筑embodied carbon图层
    map.addLayer([
        {
            "minzoom": 13,
            "layout": {
    ...
    map.on('click', (e) => {
        ...
        if (map.clickedFeatureId !== undefined) {
            map.setFeatureState(
                { source: 'sg_buildings_v2-7mhw61', id: map.clickedFeatureId },
                { clicked: false }
            );
        }
        ...
        // 设置当前建筑为点击状态
        map.setFeatureState(
            { source: 'sg_buildings_v2-7mhw61', id: featureId },
            { clicked: true }
        );
        ...
        const validEntries = Object.entries({
            ...
            "EUI (BCA)": properties['eui2020'] ? `${properties['eui2020']} kWh/m²` : null,
            "Embodied Carbon": properties['embodied_carbon'] ? `${(properties['embodied_carbon'] / 100).toPrecision(5)} 10²*kgCO2e` : null,
            "Operational Energy": properties['energy_total'] ? `${(properties['energy_total'] / 1000).toPrecision(5)} TJ` : null,
...
function showLegend(newLayer) {
    ...
    const layerVisibility = map.getLayoutProperty(layerVisibilityMap[newLayer], 'visibility');
...
function analyzePolygon(e) {
    ...
    const pointsSources = ['sg_buildings_v2-7mhw61'];
