document.addEventListener("DOMContentLoaded", function () {
    ...
    let cachedCarbonExcel = null;
    let cachedEnergyExcel = null;

    (async () => {
        const files = ['data/Asia.json', 'data/Baseline.json', 'data/PAMC.json'];

        try {
            const [datasets, carbonXlsxBuf, energyXlsxBuf] = await Promise.all([
                Promise.all(files.map(file => fetch(file).then(res => res.json()))),
                fetch("data/embodied carbon simulation details.xlsx").then(r => r.arrayBuffer()),
                fetch("data/operational carbon simulation details.xlsx").then(r => r.arrayBuffer())
            ]);
            cachedCarbonData = {
                Asia: datasets[0],
                Baseline: datasets[1],
            ...
            const wb1 = XLSX.read(carbonXlsxBuf, { type: 'array' });
            cachedCarbonExcel = XLSX.utils.sheet_to_json(wb1.Sheets["Baseline Scenario"]);
            const wb2 = XLSX.read(energyXlsxBuf, { type: 'array' });
            cachedEnergyExcel = XLSX.utils.sheet_to_json(wb2.Sheets["Sheet1"]);
    ...
    function updateArchetypes() {
        ...
        const previousArchetype = archetypeSelect.value;
        ...
        // Restore previously selected archetype if it still exists in the rebuilt list
        if (previousArchetype) {
            archetypeSelect.value = previousArchetype;
        }
    }
    ...
    function carbonsimulationDetails() {
        ...
        function updateCarbonDetails() {
            ...
            if (!cachedCarbonExcel || cachedCarbonExcel.length === 0) {
                console.warn("Embodied Carbon Excel not yet loaded");
                return;
            }

            const data = cachedCarbonExcel.find(row => row.Archetypes === selectedArchetype);
    ...
    function updateCarbonArcheChart() {
        ...
        var carbonHistogram = echarts.init(
            chartDom,
            null,
            { renderer: 'svg' }
        );
    ...
    function subCarbonInit() {
        ...
        subChart = echarts.init(
            document.getElementById('carbonSubChart'),
            null,
            { renderer: 'svg' }
        );
    ...
    function renderEnergySubChart(subtypeSelect = null) {
        const energysubChart = echarts.init(
            document.getElementById('energySubChart'),
            null,
            { renderer: 'svg' }
        );
    ...
    function energysimulationDetails() {
        ...
        function updateEnergyDetails() {
            ...
            if (!cachedEnergyExcel || cachedEnergyExcel.length === 0) {
                console.warn("Operational Carbon Excel not yet loaded");
                return;
            }

            const data = cachedEnergyExcel.find(row => row.Archetypes === selectedArchetype);
    ...
    function initEnergyChart() {
        var euiBar = echarts.init(
            document.getElementById('energy-eui'),
            null,
            { renderer: 'svg' }
        );
    ...
    // 更新图表
    function updateEnergyArcheChart() {
        let archetype = archetypeSelect.value.trim();
        const euiDom = document.getElementById('energy-eui');
        var euiBar = echarts.getInstanceByDom(euiDom) || echarts.init(euiDom, null, { renderer: 'svg' });
    ...
    carbonLink.addEventListener("click", function () {
        ...
        updateCarbonPanel();
        if (typeof updateMapFilter === 'function') updateMapFilter();
    });
    ...
    const SUNBURST_MIN_LABEL_NUMBER = 1500;
    const SUNBURST_MIN_LABEL_FOOTPRINT = 200;

    function getSunburstNodeValue(node) {
        ...
        return node.children.reduce((sum, child) => sum + getSunburstNodeValue(child), 0);
    }

    function applySunburstLabelVisibility(nodes, minValue) {
        ...
        nodes.forEach(node => {
            ...
            node.label = {
                ...(node.label || {}),
                show: nodeValue >= minValue
            };

            if (Array.isArray(node.children) && node.children.length > 0) {
                applySunburstLabelVisibility(node.children, minValue);
            }
        });
    }

    function withDynamicSunburstLabels(data, minValue) {
        const clonedData = JSON.parse(JSON.stringify(data));
        applySunburstLabelVisibility(clonedData, minValue);
        return clonedData;
    }
    ...
    typeLink.addEventListener("click", function () {
        ...
        var sunChartsNumber = echarts.init(
            sunChartsElementNumber,
            null,
            { renderer: 'svg' }
        );
        var optionone = {
            ...
            tooltip: {
                ...
                formatter: function (params) {
                    ...
                    var displayValue = formatSunburstValue(params.data.value);
                    return `${params.name}: ${displayValue} (${percentage}%)`;
                }
            },
            series: {
                type: 'sunburst',
        ...
        optionone.series.data = withDynamicSunburstLabels(optionone.series.data, SUNBURST_MIN_LABEL_NUMBER);
        sunChartsNumber.setOption(optionone);

        var sunChartsFootprint = echarts.init(
            sunChartsElementFootprint,
            null,
            { renderer: 'svg' }
        );
        var optiontwo = {
        ...
        optiontwo.series.data = withDynamicSunburstLabels(optiontwo.series.data, SUNBURST_MIN_LABEL_FOOTPRINT);
